...
/*
	...
	-> When we use a class object as an operand of operator, 
	compiler convert that expression to a function call.
*/
...
/*
	OPERATOR FUNCTIONS NEEDS TO BE 
...
/*
	operators CAN ONLY BE member operator function
	-------------------------------------------------
	[]
	->
	()
...
/*
	...
	// because of unary operators applied to only 1 object,
	// when we declare it as a member function, that object
	// is already in a hidden parameter in member functions [this pointer].
	
	// IF IT IS A GLOBAL(FREE) FUNCTION
	bool operator!(const Myclass&);

	// because global functions does not have [this pointer],
	// we need to use the operand object in the parameter.
*/
...
/*
	...
	int main()
	{
		...
		std::cout << x > 5; // syntax error [presedence of 'operator<<' is bigger than 'operator>' ]
		// std::cout.operator<<(x) > 5
		// return type of [std::cout.operator<<(x)] expression is std::cout object 
		// there is no comparison with cout object and int, syntax error happens
		...
		// operator<< is left assosiative [operator<< will start from left and goes to right]
		std::cout << bs << 5;
		// (std::cout << bs) << 5;
		operator<<(std::cout, bs).operator<<(5);
	
		// std::cout << (bs << 5);
		// bs.operator<<(5) will shift bits in bitset to left 5 times
		operator<<(std::cout, bs.operator<<(5));
	}
*/
